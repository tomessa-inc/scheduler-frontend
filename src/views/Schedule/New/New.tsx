...
injectReducer('SchedulerNew', reducer)
...
const ScheduleNew = () => {
    ...
    const { pageIndex, pageSize, sort, query, total } = useAppSelector(
        (state) => state.SchedulerNew.data.tableData
    )

    const location = useLocation()
    const navigate = useNavigate()

    const usherGroupLV = useAppSelector(
        (state) => state.SchedulerNew.data.usherGroupLV
    )

    const loading = useAppSelector(
        (state) => state.SchedulerNew.data.loading
    )
